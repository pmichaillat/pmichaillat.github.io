...
def get_recession_periods(rec_series):
    ...
    starts = rec_series.index[(~in_recession.shift(1, fill_value=0)) & in_recession]
    ends = rec_series.index[in_recession & (~in_recession.shift(-1, fill_value=0))]
...
def make_plot(df, y_column, title, filename, y_label, x_min=None, x_max=None, y_min=None, y_max=None, hline=None):
    ...
    # try:
    #     rec = DataReader('USREC', 'fred', start=df.index.min(), end=df.index.max())
    #     for start, end in get_recession_periods(rec['USREC']):
    #         fig.add_vrect(x0=start, x1=end, fillcolor='gray', opacity=0.3, line_width=0)
...
print(v_shifted.head())
print(v_shifted.info())
