...
+ Dedicated math fonts and symbols are included to typeset formulas cleanly.
...
The Source Pro family also includes a nice monospaced font: [Source Code Pro](https://mirrors.rit.edu/CTAN/fonts/sourcecodepro/doc/sourcecodepro.pdf). The template uses Source Code Pro as a monospaced font for URLs.

---

## Math fonts and symbols

If required anywhere in the CV—in titles of article or courses—mathematical expressions are typeset using the [following math fonts](/a/#math-fonts):
...
+ Blackboard-bold letters - [Fourier font](https://mirror.mwt.me/ctan/macros/latex/contrib/mathalpha/doc/mathalpha-doc.pdf) 
...
The CV title is just your name. The title is produced by the command `\name{}`. The title is typeset in small caps with a 25pt font size. 
...
The CV is organized in blocks containing key information. The blocks are ordered by importance:
...
+ Reverse-ordered list (`etaremune` environment) - Used for the article list, so articles are presented in reverse chronological order but numbered in chronological order
...
Scholars often give the full bibliographical information for their publications. That does not seem necessary: beside the name of the journal and date of publication, the rest of the information (such as journal volume, issue, DOI, and page range) is not relevant to a CV. For transparency and convenience, each publication can be linked to a webpage with the full text and full bibliographical information for interested readers.
