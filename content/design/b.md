...
The website also features [an archive](https://pascalmichaillat.org/hugo-website/archive/). The archive displays a chronological list of all papers, books, courses, and data projects. The list of sections displayed in the archive is controlled by the parameter `params:MainSections` in the `config.yml` file. The current value of that parameter is 
